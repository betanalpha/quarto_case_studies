...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-8.75 * \d, -6.76 * \d) rectangle (3.5 * \d, 4.25 * \d);

    \node (A) at (0, -6 * \d) [unobscirc] { $q_{NC_{0}}$ };

    %\node[anchor=west] at (1.1 * \d, -1 * \d) { $p(q_{C_{0}}) = \text{beta}(q_{C_{0}} \mid 12.7, 3.7)$ };
    
    \node (B) at (0, 0.5 * \d) [pushcirc] { $q_{NC, n}$ };
    ...
    \node (T) at (1.5 * \d, 2.5 * \d) [unobscirc] { $k_{\text{art}, n}$ };
    ...
    \foreach \B/\E in {A/B, B/D, C/D, B/E, D/E, 
                       F/B, G/B, H/B,
                       O/H, O/L, O/M, O/N, M/L, N/G, L/F,
                       I/F, J/G, K/H,
                       P/O, Q/N, R/M, S/L,
                       T/E} {
