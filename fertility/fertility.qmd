...
Note that these categorical variables are ordered, with a clear notion
of increasing severity.  Moreover all of these variables are indexed
from `1`, even those with only two values; this is helpful when working
with `1`-indexed programming languages like `Stan`.
...
consistency, although we don't have to be too precious.  I like to
...
```{.stan include="stan_programs/prior_tune_beta.stan" filename="prior\\\_tune\\\_beta.stan"}
```
...
$$
for $\alpha_{2} \ge \alpha_{1}$.  Similarly the conception probability
in the second least extreme group becomes
$$
q = q_{C_{0}} \, \delta_{3}
$$
...
$$
for $\alpha_{3} \ge \alpha_{2}$.  I will refer to the $\alpha_{k}$ for
...
implemented with the `simplex` variable types in Stan.  The only
...
patient using ART.  In general this probability might be coupled with
the other patient characteristic behaviors and modeling that coupling
would allow us to construct even more nuanced hypothetical cohorts, and
more precise generalized predictions.
...
https://github.com/betanalpha/quarto_case_studies/tree/main/fertility).
