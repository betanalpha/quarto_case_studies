...
Most epidemiological analyses are subject to a variety of subtle
...
used to manage these difficulties and extract productive insights.
...
The main component of the data is a collection of binary observations
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-9-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-10-1.pdf}
...
1}). Note that these categorical variables are ordered, with a clear
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-11-1.pdf}

Finally some of the patients in the observed cohort have taken advantage
of assisted reproductive technologies, or ART, which can drastically
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-12-1.pdf}
...
consistency, although we don't have to be too precious. I like to define
...
\caption{\texttt{prior\textbackslash\_tune\textbackslash\_beta.stan}}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{functions}\NormalTok{ \{}
  \CommentTok{// Differences between beta tail probabilities}
  \CommentTok{// and target probabilities}
...
\NormalTok{    deltas[}\DecValTok{1}\NormalTok{] =     beta\_cdf(theta[}\DecValTok{1}\NormalTok{] | exp(y[}\DecValTok{1}\NormalTok{]), exp(y[}\DecValTok{2}\NormalTok{])) {-} }\FloatTok{0.01}\NormalTok{;}
\NormalTok{    deltas[}\DecValTok{2}\NormalTok{] = }\DecValTok{1}\NormalTok{ {-} beta\_cdf(theta[}\DecValTok{2}\NormalTok{] | exp(y[}\DecValTok{1}\NormalTok{]), exp(y[}\DecValTok{2}\NormalTok{])) {-} }\FloatTok{0.01}\NormalTok{;}
    \ControlFlowTok{return}\NormalTok{ deltas;}
\NormalTok{  \}}
\NormalTok{\}}

\KeywordTok{data}\NormalTok{ \{}
  \DataTypeTok{real}\NormalTok{\textless{}}\KeywordTok{lower}\NormalTok{=}\DecValTok{0}\NormalTok{, }\KeywordTok{upper}\NormalTok{=}\DecValTok{1}\NormalTok{\textgreater{}     q\_low;  }\CommentTok{// Lower threshold}
  \DataTypeTok{real}\NormalTok{\textless{}}\KeywordTok{lower}\NormalTok{=q\_low, }\KeywordTok{upper}\NormalTok{=}\DecValTok{1}\NormalTok{\textgreater{} q\_high; }\CommentTok{// Upper threshold}
\NormalTok{\}}

\KeywordTok{transformed data}\NormalTok{ \{}
  \DataTypeTok{vector}\NormalTok{[}\DecValTok{2}\NormalTok{] y\_guess = [log(}\DecValTok{5}\NormalTok{), log(}\DecValTok{5}\NormalTok{)]\textquotesingle{}; }\CommentTok{// Initial guess at beta parameters}
  \DataTypeTok{vector}\NormalTok{[}\DecValTok{2}\NormalTok{] theta = [q\_low, q\_high]\textquotesingle{};    }\CommentTok{// Target quantiles}
  ...
  \CommentTok{// Find beta parameters that ensure}
  \CommentTok{// 1\% probability below lower threshold}
  \CommentTok{// and 1\% probability above upper threshold}
...
Samples were drawn using (diag_e) at Fri May  2 15:51:40 2025.
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-14-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-17-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-18-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-19-1.pdf}
...
initial model assumes homogeneous conception probabilities, the
posterior predictive conception behavior should be the same no matter
how we partition the patients. If the heterogeneity in fertility is
strong enough then the observed behaviors will fall outside of the
posterior predictive uncertainties, indicating the inadequacy of our
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-20-1.pdf}
...
systematic variation in fertility in order to adequately model this
cohort of patients.
...
q = q_{C_{0}} \, \delta_{1}
\] for \(\delta_{1} = 1\), or equivalently \[
...
q = q_{C_{0}} \, \delta_{2}
\] for \(0 \le \delta_{1} \le \delta_{2}\), or equivalently \[
q = q_{C_{0}} \, \exp(-\alpha_{2})
\] for \(\alpha_{2} \ge \alpha_{1}\). Similarly the conception
probability in the second least extreme group becomes \[
q = q_{C_{0}} \, \delta_{3}
\] for \(\delta_{2} \le \delta_{3}\), or equivalently \[
q = q_{C_{0}} \, \exp(-\alpha_{3})
\] for \(\alpha_{3} \ge \alpha_{2}\). I will refer to the \(\alpha_{k}\)
...
Samples were drawn using (diag_e) at Fri May  2 15:51:47 2025.
For each parameter, n_eff is a crude measure of effective sample size,
and Rhat is the potential scale reduction factor on split chains (at 
convergence, Rhat=1).
\end{verbatim}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-22-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-23-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-26-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-27-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-28-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-29-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-30-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-32-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-33-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-34-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-35-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-36-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-37-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-38-1.pdf}
...
All of these patient characteristic probabilities can be conveniently
implemented with the \texttt{simplex} variable types in Stan. The only
challenge is organizing all of the simplices effectively.
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-41-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-42-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-43-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-44-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-45-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-46-1.pdf}
...
In order to quantify the effect of potential interventions we need to
...
case we to reduce the population behavior into a summary amenable to
direct comparison.
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-47-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-48-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-49-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-50-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-53-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-56-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-58-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-61-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-63-1.pdf}
...
partners are unlikely to attempt one method after conception is achieved
with the other. In theory we could attempt to model these competing
events, but here we will make a simpler assumption that the two methods
of conception are approximately independent so that an observed
conception is given by a success with either method or both.

Given this assumption there are a few ways to calculate the overall
...
the other hand if we were implicitly considering domain expertise for
any method of conception then we would need to update the prior model to
...
Samples were drawn using (diag_e) at Fri May  2 15:52:14 2025.
For each parameter, n_eff is a crude measure of effective sample size,
and Rhat is the potential scale reduction factor on split chains (at 
convergence, Rhat=1).
\end{verbatim}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-65-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-68-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-69-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-70-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-71-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-72-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-73-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-74-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-77-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-79-1.pdf}
...
\includegraphics{fertility_files/figure-pdf/unnamed-chunk-82-1.pdf}
...
patient using ART. In general this probability might be coupled with the
other patient characteristic behaviors and modeling that coupling would
...
I thank Simon Steiger , Peter Alping, Joshua Entrop, Paulina Sell, and
jd for helpful comments.
...
is available on
\href{https://github.com/betanalpha/quarto_case_studies/tree/main/fertility}{GitHub}.
