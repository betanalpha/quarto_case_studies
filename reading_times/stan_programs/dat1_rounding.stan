...
model {
  ...
  for (n in 1:N) {
    ...
    real p1
      =   lognormal_cdf( reading_time[n] + 0.5 | log_mu,        phi1)
        - lognormal_cdf( reading_time[n] - 0.5 | log_mu,        phi1);
    real p2
      =   lognormal_cdf( reading_time[n] + 0.5 | log_mu + omega, phi2)
        - lognormal_cdf( reading_time[n] - 0.5 | log_mu + omega, phi2);
    ...
    if (subj_rel[n] == 0) {
      if (bernoulli_rng(lambda_SR)) {
        log_reading_time_pred[n]
          = log(round(lognormal_rng(log_mu        , phi1)));
      } else {
        log_reading_time_pred[n]
          = log(round(lognormal_rng(log_mu + omega, phi2)));
      }
    } else {
