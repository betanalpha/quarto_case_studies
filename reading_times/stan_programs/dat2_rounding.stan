...
model {
  ...
  // 100 <~ exp(nu) <~ 10000
  target += normal_lpdf(nu | 6.91, 0.99);
  ...
  for (n in 1:N) {
    ...
    real p1
      =   inv_gamma_md_cdf(reading_time[n] + 0.5 | log_mu,         phi1)
        - inv_gamma_md_cdf(reading_time[n] - 0.5 | log_mu,         phi1);
    real p2
      =   inv_gamma_md_cdf(reading_time[n] + 0.5 | log_mu + omega, phi2)
        - inv_gamma_md_cdf(reading_time[n] - 0.5 | log_mu + omega, phi2);

    if (subj_rel[n] == 1) {
      target += log(lambda_SR * p1 + (1 - lambda_SR) * p2);
    } else {
      target += log(lambda_OR * p1 + (1 - lambda_OR) * p2);
    }
  }
}
    ...
    if (subj_rel[n] == 1) {
      if (bernoulli_rng(lambda_SR)) {
        log_reading_time_pred[n]
          = log(round(inv_gamma_md_rng(log_mu,         phi1)));
      } else {
        log_reading_time_pred[n]
          = log(round(inv_gamma_md_rng(log_mu + omega, phi2)));
      }
    } else {
