...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-8.5 * \d, 0.5 * \d) rectangle (10 * \d, 10.5 * \d);

    \node (A) at (-3 * \d, 2 * \d) [obsrec] { $\boldsymbol{z}_{n}$ };
    ...
    \node (F) at (3 * \d, 2 * \d) [obsrec] { $\boldsymbol{z}_{n}$ };
    ...
    \node (L) at (-0.75 * \d, 4 * \d) [unobsell] { $\alpha, \sigma$ };
    
    \node (M) at (+0.75 * \d, 2 * \d) [unobscirc] { $\boldsymbol{\beta}$ };
    
    \node (N) at (-2 * \d, 7 * \d) [obsrec] { $\boldsymbol{z}_{\text{pred}, n}$ };
    \node (O) at (0 * \d, 7 * \d) [pushunobscirc] { $\lambda(t)$ };
    \node (P) at (0 * \d, 9 * \d) [unobscirc] { $t_{\text{pred}}$ };

    \draw[dark, line width=1.5, rounded corners=2] (-3 * \d, 6 * \d) rectangle (3 * \d, 10 * \d);        
        
    \foreach \B/\E in {L/B, L/G, L/O,
                       A/B, C/E, D/E, B/E,
                       F/G, H/J, G/J, K/J,
                       M/B, M/G, M/O,
                       N/O, O/P} {
