...
parameters {
  ...
  vector[N_entrants - 1] rel_skills_free_tilde;
...
transformed parameters {
  ...
  rel_skills[1:(anchor_entrant_idx - 1)]
    =   mu_rel_skills
      +   tau_rel_skills
        * rel_skills_free_tilde[1:(anchor_entrant_idx - 1)];
  rel_skills[anchor_entrant_idx] = 0;
  rel_skills[(anchor_entrant_idx + 1):N_entrants]
    =   mu_rel_skills
      +   tau_rel_skills
        * rel_skills_free_tilde[anchor_entrant_idx:(N_entrants - 1)];
}
...
model {
  ...
  rel_skills_free_tilde ~ normal(0, 1);
